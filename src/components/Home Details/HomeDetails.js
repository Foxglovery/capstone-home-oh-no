...
export const HomeDetails = ({ currentUser }) => {
    ...
    useEffect(() => {
        GetJobsByHomeId(currentHomeId).then((jobsArray) => {
            const numberOngoingJobs = jobsArray.filter(
                (job) => !job.endDate
            ).length;
            const stringOngoingJob = numToWord(numberOngoingJobs);
            setOngoingJobs(stringOngoingJob);
            const numberFinishedJobs = jobsArray.filter(
                (job) => job.endDate
            ).length;
            ...
            const completeImgArray = completeJobsFilter.map(
                (job) => job.imgUrl
            );
                        ...
                        <>
                            <div id="home_card">
                                ...
                                <h2 id="home_details_card_title">
                                    {home.find((h) => h.homeId === homeId)?.home
                                        .name || "Select a Home"}
                                </h2>
